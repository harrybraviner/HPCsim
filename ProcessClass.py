# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 15:27:54 2015
...
class Process:
    ...
    def mpiSend(self, SwitchList, ProcessToSendToo, packetsize):
        """This function takes the network configuaration and the process to send to
        as arguments and calls SendData to simulate the transfer between them. 
        Returning the sendingtime if a connection is established and false if it
        fails. It also calls mpiRecv to set the other processor to be recieving"""
        SwitchSend=self.parent
        SwitchGet=ProcessToSendToo.parent
        SendingTime=SendingFunctions.SendData(SwitchList,SwitchSend,SwitchGet,packetsize,self.parent.wct)
        if SendingTime:
            ProcessToSendToo.mpiRecv(SendingTime,self)
            TimeStartedSending=self.parent.wct
            TimeFinishSending=self.parent.wct+SendingTime
            return TimeFinishSending
        else:
            return False
